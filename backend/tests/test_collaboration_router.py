...
def test_session_isolation_and_concurrent_sessions():
    ...
    with client.websocket_connect("/collaboration/ws/roomA?name=UserA") as ws_a, \
...
def test_session_id_with_special_characters():
    ...
    with client.websocket_connect("/collaboration/ws/crazy-room-@#$%?name=Bhagyashri") as websocket:
        assert websocket is not None
