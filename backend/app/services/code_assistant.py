...
def run_bug_detection(code: str, language: str) -> list[dict]:
    ...
    if ai_provider.is_enabled() and found:
        try:
            asyncio.get_running_loop()
        except RuntimeError:
            found = asyncio.run(_enhance_bug_fixes_with_ai(code, language, found))

    return found


def _bug_fix_cache_key(code: str, language: str, issue: dict) -> str:
    payload = {
        "language": language,
        "type": issue.get("type"),
        "line": issue.get("line"),
        ...
        "context": issue.get("code_context"),
    }
    return json.dumps(payload, sort_keys=True, ensure_ascii=False)


def _normalize_ai_fix_text(raw_text: str) -> str:
    suggestion = " ".join(raw_text.strip().split())
    return suggestion[:280]


async def _generate_ai_bug_fix(code: str, language: str, issue: dict) -> str | None:
    context = issue.get("code_context") or issue.get("code_snippet") or code[:600]
    system_prompt = (
        "You are a senior debugging assistant. Return one concise plain-text fix suggestion only, "
        "without markdown, bullets, or code fences. Make the suggestion specific to the issue and code context."
    )
    user_prompt = (
        f"Language: {language}\n"
        ...
        f"Relevant code:\n{context}\n\n"
        "Write one actionable fix recommendation."
    )

    raw_text = await ai_provider.call_llm(system_prompt, user_prompt)
    if not raw_text:
        return None

    return _normalize_ai_fix_text(raw_text)


async def _enhance_bug_fixes_with_ai(code: str, language: str, issues: list[dict]) -> list[dict]:
    enhanced: list[dict] = []

    for issue in issues:
        cache_key = _bug_fix_cache_key(code, language, issue)
        cached = cache.get("ai-bug-fix", cache_key)
        suggestion = None
        ...
        if not suggestion:
            try:
                suggestion = await _generate_ai_bug_fix(code, language, issue)
            except Exception:
                suggestion = None

            if suggestion:
                cache.set("ai-bug-fix", cache_key, {"suggestion": suggestion})

        if suggestion:
            ai_issue = dict(issue)
            ai_issue["suggestion"] = suggestion
            ai_issue["suggestion_source"] = "ai"
            ai_issue["suggestion_tooltip"] = "This fix was generated by AI — review before applying"
            enhanced.append(ai_issue)
        else:
            rule_issue = dict(issue)
            rule_issue["suggestion_source"] = "rule"
            rule_issue["suggestion_tooltip"] = None
            enhanced.append(rule_issue)
