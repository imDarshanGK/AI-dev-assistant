...
BUG_PATTERNS: list[BugPattern] = [
    ...
    BugPattern(
        "OS System Usage",
        r"\bos\.system\s*\(",
        "`os.system()` executes shell commands directly and is highly vulnerable to command injection.",
        "Use `subprocess` with `shell=False` instead for safer execution.",
        "warning",
        ["Python"],
    ),
    BugPattern(
        "OS Popen Usage",
        r"\bos\.popen\s*\(",
        "`os.popen()` spawns a shell command and creates a pipe, risking shell injection vulnerabilities.",
        "Replace with `subprocess.Popen` or preferred high-level alternatives with arguments as a list.",
        "warning",
        ["Python"],
    ),
    BugPattern(
