...
def test_no_language_system_prompt():
    ...
    assert "QyverixAI" in prompt, "QyverixAI name missing"
    assert (
        "LANGUAGE INSTRUCTION" not in prompt
    ), "Language instruction should not be present"
...
def test_invalid_language_fallback():
    ...
    assert (
        "LANGUAGE INSTRUCTION" not in prompt
    ), "Language instruction should not be present for invalid code"
...
def test_all_supported_languages():
    ...
    for lang_code, (lang_name, lang_native) in LANGUAGE_MAP.items():
        ...
        ), f"Language instruction missing for {lang_code}"
        assert lang_name in prompt, f"Language name {lang_name} missing for {lang_code}"
        assert (
            lang_native in prompt
        ), f"Native name {lang_native} missing for {lang_code}"
        assert (
            f"respond ENTIRELY in {lang_name}" in prompt
        ), f"Language directive missing for {lang_code}"
...
def test_code_terms_exception():
    ...
    assert (
        "code-specific terms" in prompt or "return" in prompt
    ), "Code terms exception missing"
