...
from app.services.code_assistant import run_bug_detection, run_suggestions  # noqa: E402
from app.services.line_utils import (  # noqa: E402
    format_code_snippet,
...
"""
    ...
    for suggestion in suggestions:
        if (
            suggestion.get("line_number") is not None
            or suggestion.get("line_range") is not None
        ):
            has_line_refs = True
...
def test_large_code_performance():
    ...
    start = time.time()
    run_bug_detection(code, "Python")
    debug_time = time.time() - start

    start = time.time()
    run_suggestions(code, "Python")
    ...
    print(
        f"✅ test_large_code_performance PASSED (debug: {debug_time:.3f}s, suggest: {suggest_time:.3f}s)"
    )
